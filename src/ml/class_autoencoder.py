...
class AutoencoderTrainer:
    ...
    def evaluate(
        ...
        dataloader, processed_data = self.create_test_loader(
    ...
    def create_data_loaders(
        ...
        """
        # Create an AcornData instance for data processing
        acorn_data = AcornData(acorn_group="Comfortable")

        return acorn_data.create_data_loaders(
            processed_data, batch_size, num_workers, validation_split
        )
    ...
    def create_test_loader(
        ...
        # Use the AcornData class to preprocess and create the test loader
        return acorn_data.create_test_loader(data, batch_size, num_workers)
...
class AutoencoderPipeline:
    ...
    def __init__(
        ...
        # Initialize components - use AcornData instead of DataProcessor
        self.acorn_data = AcornData(acorn_group=self.acorn_group, selected_years=self.selected_years)

    def load_data(self):
        """Load and preprocess data from the Acorn dataset."""
        start_time = time.time()
        # Get data from AcornData
        data = self.acorn_data.get_data()
        self.original_data = data.select("hh_consumption").to_numpy()
        logging.info(f"Data loading took {time.time() - start_time:.2f} seconds")

        # Process data using AcornData
        self.processed_data = self.acorn_data.preprocess_data(self.original_data)
        self.input_dim = (
