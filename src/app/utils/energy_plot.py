...
def create_aggregated_plot(df, x_field, metric, metric_display, time_period, energy_unit, color_field):
    """
...
def create_by_year_plot(df, x_field, metric, metric_display, time_period, energy_unit, color_field, separate_years):
    """
    ...
    if separate_years and time_period == "Seasonal":
        ...
        fig.for_each_annotation(lambda a: a.update(text=a.text.split("=")[-1]))  # Simplify titles
    else:
        fig = px.line(
            df,
            x=x_field,
...
def create_energy_line_plot(
    ...
    group_fields = validate_columns(data, group_fields, default=[x_field] if x_field in data.columns else None)
    ...
    if display_type == "Aggregated":
        ...
        fig = create_aggregated_plot(df, x_field, metric, metric_display, time_period, energy_unit, color_field)
    else:  # By Year
        df = prepare_by_year_data(data, x_field, metric, group_fields, sort_by, color_field)
