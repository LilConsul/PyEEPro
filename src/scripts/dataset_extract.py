...
def extract_dataset(
    ...
    try:
        ...
        if progress_callback:
            progress_callback(
                0.25, f"Starting extraction from {dataset_zip_path.name}..."
            )
        ...
        try:
            with zipfile.ZipFile(dataset_zip_path, "r") as zip_ref:
                ...
                if total_files == 0:
                    if progress_callback:
                        progress_callback(
                            0.0, f"ZIP file is empty: {dataset_zip_path.name}"
                        )
                    return
                ...
                if progress_callback:
                    progress_callback(
                        0.3,
                        f"Found {total_files} files to extract ({total_size / 1024 / 1024:.1f} MB total)",
                    )
                ...
                for i, file in enumerate(files):
                    try:
                        ...
                        if not silent and i % 20 == 0:
                            print(
                                f"Progress: {extracted_files}/{total_files} files extracted"
                            )
                    except Exception as e:
                        # Log the error but continue with other files
                        print(
                            f"Error extracting file {file.filename}: {e}",
                            file=sys.stderr,
                        )
                        if progress_callback:
                            progress_callback(
                                progress,
                                f"Warning: Error extracting {file.filename}: {e}",
                            )
        ...
        if progress_callback:
            progress_callback(
                1.0,
                f"Extraction complete! {extracted_files} files extracted to {output_dir.name}",
            )
